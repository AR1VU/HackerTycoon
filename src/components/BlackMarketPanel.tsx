...
import { ShoppingCart, Package, Star, Lock, CheckCircle, Coins } from 'lucide-react';
...
const BlackMarketPanel: React.FC<BlackMarketPanelProps> = ({
  blackMarket,
  playerInventory,
  cryptoWallet,
  isOpen,
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'exploits' | 'upgrades' | 'gear' | 'scripts'>('all');
  ...
  const getFilteredItems = () => {
    switch (activeTab) {
      case 'exploits': return blackMarket.items.filter(item => item.type === 'exploit');
      case 'upgrades': return blackMarket.items.filter(item => item.type === 'tool_upgrade');
      case 'gear': return blackMarket.items.filter(item => item.type === 'gear');
      case 'scripts': return blackMarket.items.filter(item => item.type === 'script');
      default: return blackMarket.items;
    }
  };
  ...
  const availableItems = filteredItems.filter(item => !item.owned);
      ...
      <div className="bg-black border-2 border-red-400 rounded-lg w-full max-w-6xl h-[90vh] mx-4 shadow-2xl shadow-red-500/20 flex flex-col">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="flex border-b border-red-400/30 bg-black/50">
              {[
                { key: 'all', label: 'All Items', count: blackMarket.items.length },
                { key: 'exploits', label: 'Exploits', count: blackMarket.items.filter(i => i.type === 'exploit').length },
                { key: 'upgrades', label: 'Upgrades', count: blackMarket.items.filter(i => i.type === 'tool_upgrade').length },
                { key: 'gear', label: 'Gear', count: blackMarket.items.filter(i => i.type === 'gear').length },
                { key: 'scripts', label: 'Scripts', count: blackMarket.items.filter(i => i.type === 'script').length }
              ].map(tab => (
                <button
                  key={tab.key}
            ...
            <div className="flex-1 p-4 overflow-y-auto scrollbar-thin scrollbar-thumb-red-600 scrollbar-track-black">
              ...
              {availableItems.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-red-300 font-mono text-lg font-bold mb-4 flex items-center">
              ...
              {ownedItems.length > 0 && (
                <div>
                  <h3 className="text-green-400 font-mono text-lg font-bold mb-4 flex items-center">
              ...
              {filteredItems.length === 0 && (
                <div className="text-center text-red-300 mt-8">
                  ...
                  <p className="text-red-400">Check back later for new inventory.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="p-4 border-t border-red-400/30 bg-black/50">
          <div className="text-center text-red-400 text-sm font-mono">
            🔒 All transactions are encrypted and untraceable • Use 'inventory' command to view owned items
          </div>
        </div>
      </div>
    </div>
